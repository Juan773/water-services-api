...
from water_services_api.apps.operation.serializers.Quota import QuotaBasicSerializer


class PaymentSerializer(serializers.ModelSerializer):
    client = ClientBasicSerializer(many=False, read_only=True)
    quota = QuotaBasicSerializer(many=False, read_only=True)
    payment_method = PaymentMethodSerializer(many=False, read_only=True)

    class Meta:
        model = Payment
        fields = (
            'id', 'client', 'client_id', 'date', 'quota', 'quota_id', 'payment_method', 'payment_method_id',

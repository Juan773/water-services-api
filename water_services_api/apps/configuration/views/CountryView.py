...
from water_services_api.apps.configuration.permissions.Country import CountryPermissions as DisaryPermission
from water_services_api.apps.configuration.serializers.Country import CountrySerializer
...
class CountryViewSet(CustomPagination, DefaultViewSetMixin, viewsets.ModelViewSet):
    queryset = Country.objects.all()
    serializer_class = CountrySerializer
    search_fields = ('name', 'code', 'abbreviation', 'phone_code',)
    ordering_fields = ('code', 'name', 'abbreviation', 'phone_code',)
    ...
    def edit(self, request):
        try:
            ...
            model = Country.objects.get(pk=id)
    ...
    def update_(self, request, pk=None):
        data = request.data
        try:
            p = Country.objects.filter(pk=data['id']).update(**data)
            model = Country.objects.get(id=data['id'])
            result = parse_success(
